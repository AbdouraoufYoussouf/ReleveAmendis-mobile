  ...
  const p = new Promise((resolve, reject) => {
    db.transaction(
      tx => {
        ...
        tx.executeSql(`CREATE TABLE IF NOT EXISTS rue ( numeroRue VARCHAR(4) PRIMARY KEY NOT NULL UNIQUE,titreRue VARCHAR(15), nomRue TEXT, numeroTourne VARCHAR(20), codeSecteur VARCHAR(10) NOT NULL,FOREIGN KEY(numeroTourne) REFERENCES tournee(numeroTourne) ON DELETE SET NULL ON UPDATE CASCADE, FOREIGN KEY(codeSecteur) REFERENCES secteur(codeSecteur) ON DELETE CASCADE ON UPDATE CASCADE);`, [], onSuccess, onError);
        ...
        tx.executeSql(`CREATE TABLE IF NOT EXISTS anomalie (codeAnomalie INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,designation VARCHAR(50),libele VARCHAR(5),codeFluide VARCHAR(20),FOREIGN KEY(codeFluide) REFERENCES fluide(codeFluide) ON DELETE SET NULL ON UPDATE CASCADE);`, [], onSuccess, onError);
        ...
        tx.executeSql(`CREATE TABLE IF NOT EXISTS compteur (compteurId INTEGER PRIMARY KEY AUTOINCREMENT,
          numeroCompteur VARCHAR(50) UNIQUE,idGeographique VARCHAR(10),
          nomAbonne VARCHAR(50),police VARCHAR(15),adresse TEXT,
          ancienIndex INTEGER,nouveauIndex INTEGER,etatLecture VARCHAR(2),
          consommation INTEGER,consMoyenne INTEGER, dateReleve TEXT,
          heureReleve TEXT,nouveauIndex1 INTEGER, nouveauIndex2 INTEGER, 
          nouveauIndex3 INTEGER,nouveauIndex4 INTEGER, nouveauIndex5 INTEGER,
          nouveauIndex6 INTEGER, nouveauIndex7 INTEGER, codeFluide VARCHAR(20),
          numeroRue VARCHAR(4), codeSecteur VARCHAR(10), anomalie1 INTEGER ,
          anomalie2 INTEGER ,
          ordreRue INTEGER ,
        ...
        // *********** INSERT SEED DATA ***************//
        tx.executeSql(`INSERT INTO terminal(terminalId,numeroTPL,isCreatec) VALUES(1,'TPL1',1);`, [], onSuccess, onError);
...
export const seedDatabase = () => {
  ...
  const p = new Promise((resolve, reject) => {
    db.transaction(
      tx => {
        ...
        tx.executeSql(`CREATE TABLE IF NOT EXISTS compteur (compteurId INTEGER PRIMARY KEY AUTOINCREMENT,
          ...
          adresse TEXT,ancienIndex INTEGER,nouveauIndex INTEGER,etatLecture VARCHAR(2),consommation INTEGER,
          ...
          nouveauIndex3 INTEGER,nouveauIndex4 INTEGER, nouveauIndex5 INTEGER, nouveauIndex6 INTEGER,
           nouveauIndex7 INTEGER, codeFluide VARCHAR(20),numeroRue VARCHAR(4), codeSecteur VARCHAR(10), 
           ordreRue INTEGER ,
           anomalie1 INTEGER ,
           anomalie2 INTEGER ,codeEtat VARCHAR(2),
           numeroTourne VARCHAR(20) ,
           FOREIGN KEY (numeroTourne) REFERENCES tournee(numeroTourne) ON DELETE SET NULL ON UPDATE CASCADE,
           FOREIGN KEY (anomalie1) REFERENCES anomalie(codeAnomalie) ON DELETE SET NULL ON UPDATE CASCADE,
    ...
    tx.executeSql(`INSERT INTO compte(email,password,userId) VALUES
      ...
      () => {
        ToastSuccess(' Creation Db et Inserssion des donnés reussi!')
        console.log(`TX OK.`);
